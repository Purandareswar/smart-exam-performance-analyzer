...
df["rank"] = df["calculated_avg"].rank(ascending=False, method="dense")
...
pass_count = (filtered_df["status"] == "Pass").sum()
fail_count = total - pass_count

c1, c2, c3 = st.columns(3)
c1.metric("👨‍🎓 Students", total)
c2.metric("✅ Pass %", f"{(pass_count/total)*100:.1f}%" if total else "0%")
c3.metric("❌ Fail %", f"{(fail_count/total)*100:.1f}%" if total else "0%")
...
with col1:
    ...
    fig1.update_layout(title="Pass vs Fail")
    st.plotly_chart(fig1, use_container_width=True)

with col2:
    subject_avg = {sub: filtered_df[sub].mean() for sub in subjects}
    fig2 = px.bar(
        x=list(subject_avg.keys()),
        y=list(subject_avg.values()),
        text=[round(v, 1) for v in subject_avg.values()],
        color=list(subject_avg.values()),
        color_continuous_scale="Blues"
    )
    fig2.update_traces(textposition='outside')
    fig2.update_layout(title="Subject Averages")
    st.plotly_chart(fig2, use_container_width=True)

# ---------- TOPPER ----------
if not filtered_df.empty:
    topper = filtered_df.loc[filtered_df["calculated_avg"].idxmax()]
    ...
    st.image(
        f"https://ui-avatars.com/api/?name={topper['name']}&background=222&color=fff",
        width=120
    )
...
if st.button("Analyze"):
    ...
    if student.empty:
        ...
        def generate_pdf():
            ...
            content.append(Paragraph("<b><font size=18>Student Performance Report</font></b>", styles['Title']))
            content.append(Spacer(1, 15))

            content.append(Paragraph("<b>Student Details</b>", styles['Heading2']))
            content.append(Spacer(1, 10))

            content.append(Paragraph(f"<b>Name:</b> {student['name']}", styles['Normal']))
            content.append(Paragraph(f"<b>ID:</b> {student['student_id']}", styles['Normal']))
            content.append(Paragraph(f"<b>Average Score:</b> {avg:.2f}%", styles['Normal']))

            content.append(Spacer(1, 15))

            content.append(Paragraph("<b>Subject-wise Marks</b>", styles['Heading2']))
            content.append(Spacer(1, 10))

            for s, m in marks.items():
                content.append(Paragraph(f"<b>{s}:</b> {m}", styles['Normal']))

            content.append(Spacer(1, 15))

            content.append(Paragraph("<b>Performance Summary</b>", styles['Heading2']))
            content.append(Spacer(1, 10))
            ...
            content.append(Paragraph(f"<b>Overall Performance:</b> {performance}", styles['Normal']))

            weak = min(marks, key=marks.get)
            strong = max(marks, key=marks.get)

            content.append(Paragraph(f"<b>Strong Subject:</b> {strong}", styles['Normal']))
            content.append(Paragraph(f"<b>Needs Improvement:</b> {weak}", styles['Normal']))

            content.append(Spacer(1, 20))

            content.append(Paragraph("<i>Generated by Smart Exam Analyzer</i>", styles['Normal']))
        ...
        st.download_button("📄 Download Professional Report", pdf, "Student_Report.pdf")